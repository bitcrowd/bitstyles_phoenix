defmodule BitstylesPhoenix.Component.Breadcrumbs do
  ...
  story(
    "Breadcrumbs with custom classes and propertires",
    '''
        iex> assigns = %{}
        ...> render ~H"""
  ...
  def ui_breadcrumbs(assigns) do
    extra = assigns_to_attributes(assigns, [:icon_file, :item, :class])
    ...
    ~H"""
      <nav aria-label="breadcrumbs" {extra}>
        <ol class={classnames(["u-h6 a-list-reset u-flex u-flex-wrap u-items-center", assigns[:class]])}>
          <%= for item <- @item do %>
            <li
              class={classnames(["u-margin-xs-right u-flex u-items-center", item[:class]])}
