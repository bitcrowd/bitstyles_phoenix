defmodule BitstylesPhoenix.Form do
  ...
  story("Text field with label", '''
      iex> render ui_input(@user_form, :name)
      """
      <label for="user_name">
        Name
      </label>
  ...
  story("Text field with error", '''
      iex> render ui_input(@error_form, :name)
      """
      <label for="user_name">
        Name
      </label>
      <input id="user_name" maxlength="255" name="user[name]" type="text"/>
      <span class="u-fg--warning" phx-feedback-for="user_name">
        is too short
      </span>
      """
  ''')
  ...
  story("Text field with multiple errors", '''
      iex> render ui_input(@error_form, :email)
      """
      ...
      <ul class=\"u-padding-l-left u-fg--warning\">
        <li>
          <span class=\"u-fg--warning\" phx-feedback-for=\"user_email\">
            is invalid
          </span>
        </li>
        <li>
  ...
  story("Text field with hidden label", '''
      iex> render ui_input(@user_form, :name, hidden_label: true)
      """
  ...
  story("Text field with options", '''
      iex> render ui_input @user_form, :totp, label: "Authentication code", placeholder: "6-digit code", required: true, value: "", inputmode: "numeric", pattern: "[0-9]*", autocomplete: "one-time-code", maxlength: 6, label_opts: [class: "extra"]
      """
  ...
  story("Email field with label", '''
      iex> render ui_input(@user_form, :email, type: :email)
      """
      <label for="user_email">
        Email
      </label>
  ...
  story("Search field with placholder", '''
      iex> render ui_input(@user_form, :email_or_name, type: :search, placeholder: "Search by email or name", autofocus: true)
      """
  ...
  story("File field for pdfs", '''
      iex> render ui_input(@file_form, :file, type: :file, accept: "application/pdf")
      """
      <label for="user_file">
        File
      </label>
  ...
  story("Checkbox", '''
      iex> render ui_input(@user_form, :accept, type: :checkbox)
      """
      <label>
  ...
  story("Radio", '''
      iex> render ui_input(@user_form, :option, type: :radio, value: "one", label: "One")
      """
      <label>
        <input id="user_option_one" name="user[option]" type="radio" value="one"/>
        One
      </label>
      """
  ''')
  ...
  story("Textarea", '''
      iex> render ui_textarea(@user_form, :about_me)
      """
  ...
  story("Textarea with options", '''
      iex> render ui_textarea(@user_form, :metadata, label: "Metadata", value: "Value here", rows: 10, style: "height: auto;", label_opts: [class: "extra"])
      """
      <label class="extra" for="user_metadata">
        Metadata
      </label>
      <textarea id="user_metadata" name="user[metadata]" rows="10" style="height: auto;">
        Value here
      </textarea>
      """
  ''')
  ...
  story("Textarea with hidden label", '''
      iex> render ui_textarea(@user_form, :address, hidden_label: true)
      """
  ...
  story("Select box", '''
      iex> render ui_select(@user_form, :week, 1..2)
      """
      <label for="user_week">
        Week
      </label>
  ...
  story("Select box without label", '''
      iex> render ui_select(@user_form, :week, 1..2, hidden_label: true)
      """
  ...
  story("Select box with options", '''
      iex> render ui_select(@user_form, :preference, [{"Ducks", "ducks"}, {"Cats", "cats"}], label: "What do you like best?", label_opts: [class: "extra"])
      """
